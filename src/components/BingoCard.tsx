...
export function BingoCard(props: BingoCardProps) {
  const { grid, gridSize, markedCells, onCellClick, winningLines = [], hideText = false } = props;
      ...
      {filledGrid.map((item, index) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative flex items-center justify-center rounded-lg border-2 p-2 text-center text-[8px] font-medium transition-all duration-200",
              !hideText && "hover:scale-105 sm:text-xs",
              isMarked
